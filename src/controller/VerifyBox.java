...
public class VerifyBox extends MaxObject {
    ...
    private int handInBox(int tx, int ty, int tsize) {
	float mean = 0f;
	for (int i=0;i<NUM_FRAME;i++) {
	    mean += frame_score_hist[i];
	}
	mean = mean / (float)NUM_FRAME;
	if (mean > 1.5f) return 1;
	else if (mean < -1.5f) return -1;
	else return 0;
    }
	...
	*/
	...
	// input video with face
  	if (inlet_no == 0) {
		...
		if (hand_in_box && (handInBox(box_x, box_y, box_size) == -1)) {
		    hand_in_box = false;
		    color = red;
		    outlet(1, "hand_out");
		...
		} else if (!hand_in_box && (handInBox(box_x, box_y, box_size) == 1)) {
		    hand_in_box = true;
		    color = blue;
		    outlet(1, "hand_in");
		    handin_count++;
	...
	disp("Please sit down on the couch and try to pop bubbles with your hand on the left screen. To stop recording, place your hand on the stop button.");
