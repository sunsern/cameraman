public class IntegralImage {
	...
	public double getVariance(int x, int y, int size) {
		double v;
		v = this.getMean(x, y, size);
		v = this.getMeanSquare(x, y, size) - v * v;
	...
	public IntegralImage(byte[] buf, int w, int h, boolean sumSquareNeeded) {
		...
		if (ss)
			compute_ii_with_iis(buf);
		else
			compute_ii_without_iis(buf);
	}
