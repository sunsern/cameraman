...
public class FaceDetectOld {
	...
	private static double thresholdDetection = 15.0d;
	
	private static double thresholdFace = 15.0d;
	private static double thresholdNonFace = -10.0d;
	private static double hardMargin = 4d;
	
	private static double probToAddFace = 1.0;
	private static double probToAddNonFace = 0.000;
	private static double probToAddHardExamples = 0.00003;
	...
	private static String NONFACE_DIR = "nonface"; 
	...
	private static Vector<ExtendedCandidate> faces, candidates;
	private static BufferedWriter scoreVectorFile = null;
	
	
	public FaceDetectOld(String xml_filename, String skin_xml) throws ParserConfigurationException, SAXException, IOException {
	...
	public static void main(String[] args) {
		...
		try {
			...
			make_dir(facePath);
			String nonfacePath = input_dir + File.separator + NONFACE_DIR + File.separator;
			make_dir(nonfacePath);
			...
			FaceDetectOld fd = new FaceDetectOld(xml_filename, SKIN_XML);
			...
			for (int i = 0; i < children.length; i++) {
				...
				candidates = new Vector<ExtendedCandidate>();
				faces = new Vector<ExtendedCandidate>();
				...
				for (int j=0;j<candidates.size();j++) {
					...
					if (c.score > thresholdFace) {
						double roll = Math.random();
						if (roll < probToAddFace) {
							String filename = input_dir + "_" + image_filename + "_" + i + "_" + detection_count + "_" + 
	            (int)(Math.random()*10000) + "-" + c.score + ".png"; 
							BufferedImage bout = fd.current_bimg.getSubimage(c.x, c.y, c.size, c.size);
							ImageIO.write(bout, "png", new File(facePath + filename));
						}
					}
					else if (c.score < thresholdNonFace) {
						double roll = Math.random();
						if (roll < probToAddNonFace) {
							String filename = input_dir + "_" + image_filename + "_" + i + "_" + detection_count + "_" + 
	            (int)(Math.random()*10000) + "-" + c.score + ".png"; 
							BufferedImage bout = fd.current_bimg.getSubimage(c.x, c.y, c.size, c.size);
							ImageIO.write(bout, "png", new File(nonfacePath + filename));
						}
					}
					else if (c.score > -hardMargin && c.score < hardMargin) {
						double roll = Math.random();
						if (roll < probToAddHardExamples) {
							String filename = input_dir + "_" + image_filename + "_" + i + "_" + detection_count + "_" + 
	            (int)(Math.random()*10000) + "-" + c.score + ".png"; 
							BufferedImage bout = fd.current_bimg.getSubimage(c.x, c.y, c.size, c.size);
							ImageIO.write(bout, "png", new File(hardPath + filename));
						}
					}
				...
				for (int j=0;j<faces.size();j++) {
					...
					for (int yy=c.y;yy < c.y+c.size; yy++) {
						for (int xx=c.x;xx < c.x+c.size; xx++) {
							...
							fd.packedRgbs[yy*w+xx] = ((r<<16) | (g<<8) | b);
						}
					}

					// add red box in the center
					int center_x = c.x + c.size/4;
					int center_y = c.y + c.size/4;
					int center_size = 4 * c.size / 8;
          ...
          (int)(Math.random()*10000) + "-" + c.score + ".png"; 
				...
				// output
				fd.current_bimg.setRGB(0, 0, w, h, fd.packedRgbs, 0, w);
	...
	private static void calibrate_face(FaceDetectOld fd) {
		
		Vector<ExtendedCandidate> output = new Vector<ExtendedCandidate>();
		Vector<ExtendedCandidate> high_score = new Vector<ExtendedCandidate>();
	...
	private static class FaceThread extends Thread {
		...
		public FaceThread(FaceDetectOld fd, int s, int w, int h) {
			this.fd = fd;
			size = s;
			fw = w;
			fh = h;
