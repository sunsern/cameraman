...
public class FaceCreateXML {
	
	private Node[] nodes;
	private Stack<ParentInfo> parent;

	public FaceCreateXML(String tree_filename) throws IOException {
	...
	private static String getFeatureType(String s) {
		if (s.contains(HogFeature.NAME)) return HogFeature.NAME;
		else if (s.contains(SkinFeature.NAME)) return SkinFeature.NAME;
		else if (s.contains(GrayFeature.NAME)) return GrayFeature.NAME;
		else if (s.contains(RootFeature.NAME)) return RootFeature.NAME;
		else if (s.contains(HsvSkinFeature.NAME)) return HsvSkinFeature.NAME;
	...
	public void createTree(String tree_file_name) throws IOException {
		...
		while (aline != null && aline.length() > 0) {
			...
			if (input_line[2].startsWith("Splitter")) {
				...
				if (ftype.compareTo(SkinFeature.NAME) == 0) {
					...
					nodes[boosting_num] =
						new Node(boosting_num, current_parent.id, current_parent.cond,
						         findex, Double.parseDouble(input_line[7]), SkinFeature.NAME);
				}
					...
					nodes[boosting_num] =
						new Node(boosting_num, current_parent.id, current_parent.cond,
						         findex, Double.parseDouble(input_line[7]), GrayFeature.NAME);
				}
					...
					nodes[boosting_num] =
						new Node(boosting_num, current_parent.id, current_parent.cond,
						         findex, Double.parseDouble(input_line[7]), HogFeature.NAME);
				}
					...
					nodes[boosting_num] =
						new Node(boosting_num, current_parent.id, current_parent.cond,
						         findex, Double.parseDouble(input_line[7]), HsvSkinFeature.NAME);
				}
				else {
	...
	public void printXML(BufferedWriter bw) throws IOException {
		...
		for (int i = 1; i < nodes.length; i++) {
			if (nodes[i].type.compareTo(HogFeature.NAME) == 0) {
				bw.write("\t\t\t<feature id=\"" + i + "\" type=\"" + HogFeature.NAME + "\">\n");
				bw.write("\t\t\t\t<_>" + nodes[i].findex + "</_>\n");	
			}
			else if (nodes[i].type.compareTo(SkinFeature.NAME) == 0) {
				bw.write("\t\t\t<feature id=\"" + i + "\" type=\"" + SkinFeature.NAME + "\">\n");
				bw.write("\t\t\t\t<_>" + nodes[i].findex + "</_>\n");
			}
			else if (nodes[i].type.compareTo(HsvSkinFeature.NAME) == 0) {
				bw.write("\t\t\t<feature id=\"" + i + "\" type=\"" + HsvSkinFeature.NAME + "\">\n");
				bw.write("\t\t\t\t<_>" + nodes[i].findex + "</_>\n");
			}
			else if (nodes[i].type.compareTo(GrayFeature.NAME) == 0) {
				bw.write("\t\t\t<feature id=\"" + i + "\" type=\"" + GrayFeature.NAME + "\">\n");
				bw.write("\t\t\t\t<_>" + nodes[i].findex + "</_>\n");
			}
	...
	public static void main(String args[]) {
		...
		if (args.length != 2) {
			System.out.
				println("usage: java FaceCreateXML <tree-filename> <new XML>");
			return;
		}
		...
		try {
			FaceCreateXML fc = new FaceCreateXML(tree_filename);
